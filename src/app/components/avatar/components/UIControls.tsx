...
const UIControls: React.FC<UIControlsProps> = ({
  ...
  return (
    <>
      ...
      {/* Mensaje de instrucción para pantalla completa visible solo en ese modo */}
      {isFullscreen && (
        <div className={`absolute top-4 left-1/2 transform -translate-x-1/2 ${theme === "dark" ? "bg-gray-900/80 text-white border-gray-700" : "bg-white/80 text-gray-800 border-gray-300"} px-3 py-2 rounded-md text-sm border z-10 transition-opacity duration-300`} style={{ opacity: isFullscreen ? 1 : 0 }}>
          <p>Presiona <kbd className={`px-1.5 py-0.5 rounded ${theme === "dark" ? "bg-gray-800" : "bg-gray-200"} font-mono text-xs mx-1`}>ESC</kbd> o el botón para salir de pantalla completa</p>
        </div>
      )}
    </>
  );
